...
describes what users get. Pick a format with `-o`, or let batdongsan choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
batdongsan <command> -o table   # aligned columns for reading
batdongsan <command> -o jsonl   # one JSON object per line, for piping
batdongsan <command> -o json    # a single JSON array
batdongsan <command> -o csv     # spreadsheet friendly
batdongsan <command> -o tsv     # tab-separated
batdongsan <command> -o url     # just the URL column
batdongsan <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
batdongsan <command> --fields id,title,url
```
...
```bash
batdongsan <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
batdongsan <command>            # a table, because this is a terminal
batdongsan <command> | wc -l    # JSONL, because this is a pipe
```
